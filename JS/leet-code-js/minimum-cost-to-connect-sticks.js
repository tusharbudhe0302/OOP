...
const sortArray = (list) => {
    return list.sort((a, b) => a - b).reduce((ObjectMap, n, i) => {
        console.log(`ObjectMap : ${JSON.stringify(ObjectMap)} n : ${n} i :  ${i} current list after reduce : ${list}`);
        if (i == 0) { ObjectMap.previous = n; return ObjectMap; };
        const sumv2 = ObjectMap.previous + n;
        ObjectMap.previous = sumv2;
